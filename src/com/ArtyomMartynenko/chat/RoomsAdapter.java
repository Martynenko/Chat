...
public class RoomsAdapter extends BaseAdapter {
	...
	RoomsAdapter(Context c, List <Room> l){
		mC=c;
		mL=l;
	}
...
//		nameView.setText(item.name);
		...
		if(item.getStatus().toString().equals("ok")){
			((ImageView)maskedView.findViewById(R.id.row_status)).setImageResource(android.R.drawable.ic_media_play);
		}
		if(item.getStatus().toString().equals("inside")){
			((ImageView)maskedView.findViewById(R.id.row_status)).setImageResource(android.R.drawable.ic_lock_lock);
		}
		if(item.getStatus().toString().equals("banned")){
			((ImageView)maskedView.findViewById(R.id.row_status)).setImageResource(android.R.drawable.ic_delete);
		}
